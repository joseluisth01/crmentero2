 ...
 * MODIFICADO:
 * - Una sola sección de cláusulas.
 * - Checkbox Kit Digital cambia qué cláusulas estándar se cargan.
 * - Descripción de servicios contratados como editor WYSIWYG Quill.
 */
...
<style>
    ...
    .wysiwyg-container { border:2px solid #e0e0e0; border-radius:8px; overflow:hidden; transition:border-color .3s; background:white; }
    ...
    .item-desc-editor-wrap .ql-container { min-height:75px; font-size:13px; }
    .item-desc-editor-wrap .ql-editor { min-height:75px; padding:10px; }
    ...
    <form id="contratoForm" class="editor-container">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" name="fecha_contrato" required
                        value="<?php echo $contrato ? htmlspecialchars($contrato['fecha_contrato'] ?? '') : date('Y-m-d'); ?>">
                </div>

                <div class="form-group">
                    <label>Válido Hasta *</label>
                    <input type="date" name="valido_hasta" required
                        value="<?php echo $contrato ? htmlspecialchars($contrato['valido_hasta'] ?? '') : date('Y-m-d', strtotime('+1 year')); ?>">
                </div>
            </div>
        </div>
                ...
                <?php foreach ($existingItems as $idx => $item): ?>
                    <div class="item-row" data-item-index="<?php echo $idx; ?>">
                        <div class="form-group">
                            ...
                            <div class="ss-wrapper" id="artWrapper_<?php echo $idx; ?>">
                                ...
                                <div class="ss-dropdown" id="artDropdown_<?php echo $idx; ?>">
                                    <div class="ss-search">
                                        <input type="text" id="artSearch_<?php echo $idx; ?>" placeholder="Buscar artículo..." autocomplete="off">
                                    </div>
                            ...
                            <input type="text"
                                name="items[<?php echo $idx; ?>][nombre]"
                            ...
                            <input type="hidden"
                                name="items[<?php echo $idx; ?>][descripcion]"
                                class="item-descripcion-hidden"
                                value="<?php echo $item ? htmlspecialchars($item['descripcion'] ?? '', ENT_QUOTES, 'UTF-8') : ''; ?>">

                            <div class="wysiwyg-container item-desc-editor-wrap" style="margin-top:10px;">
                                <div id="itemDescEditor_<?php echo $idx; ?>" class="item-desc-editor"></div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number"
                                name="items[<?php echo $idx; ?>][cantidad]"
                                placeholder="1"
                                step="0.01"
                                required
                                class="item-cantidad"
                                value="<?php echo $item ? htmlspecialchars($item['cantidad'] ?? 1) : 1; ?>">

                            <input type="text"
                                name="items[<?php echo $idx; ?>][unidad]"
                                placeholder="Mensual, Único..."
                                class="item-unidad"
                        ...
                        <div class="form-group">
                            ...
                            <input type="number"
                                name="items[<?php echo $idx; ?>][precio]"
                                placeholder="0.00"
                                step="0.01"
                                required
                                class="item-precio"
                                value="<?php echo $item ? htmlspecialchars($item['precio'] ?? '') : ''; ?>">

                            <div class="precio-original-wrap">
                                <label>Precio original sin descuento (€) — opcional</label>

                                <input type="number"
                                    name="items[<?php echo $idx; ?>][precio_original]"
                                ...
                                <div class="precio-original-preview"
                                    id="prevOriginal_<?php echo $idx; ?>"
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="iva" id="iva" step="0.01" min="0"
                        value="<?php echo $contrato ? htmlspecialchars($contrato['iva'] ?? 21) : 21; ?>">
                </div>

                <div class="form-group">
                    <label>Segundo Impuesto (%) — Opcional</label>
                    <input type="number" name="segundo_impuesto" id="segundoImpuesto" step="0.01" min="0"
                        value="<?php echo $contrato ? htmlspecialchars($contrato['segundo_impuesto'] ?? 0) : 0; ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="descuento_global" id="descuentoGlobal" step="0.01" min="0"
                        value="<?php echo $contrato ? htmlspecialchars($contrato['descuento_global'] ?? 0) : 0; ?>">
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="clausulas_html" id="clausulasHidden" value="<?php echo $contrato ? htmlspecialchars($contrato['clausulas_html'] ?? '', ENT_QUOTES, 'UTF-8') : ''; ?>">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <input type="hidden" name="notas" id="notasHidden" value="<?php echo $contrato ? htmlspecialchars($contrato['notas'] ?? '', ENT_QUOTES, 'UTF-8') : ''; ?>">
...
<script>
const CLAUSULAS_NORMAL = <?php echo json_encode($clausulasNormal, JSON_UNESCAPED_UNICODE); ?>;
const CLAUSULAS_KIT_DIGITAL = <?php echo json_encode($clausulasKitDigital, JSON_UNESCAPED_UNICODE); ?>;
...
// ── DATA ───────────────────────────────────────────────────────
let itemCounter = <?php echo intval(count($existingItems)); ?>;
const articulosData = <?php echo $articulosJson ?: '[]'; ?>;
const clientesData  = <?php echo $clientesJson ?: '[]'; ?>;
const selClienteId  = <?php echo json_encode((string)$selectedClienteId); ?>;
...
function makeSearchableSelect(cfg) {
    ...
    if (!main || !drop || !search || !optsEl || !txtEl) return null;
    ...
    function render(q) {
        ...
        if (!filtered.length) {
            optsEl.innerHTML = '<div class="ss-none">Sin resultados</div>';
            return;
        }
        ...
        optsEl.querySelectorAll(".ss-option").forEach((el, i) => {
    ...
    function pick(i) {
        ...
        txtEl.textContent = cfg.label(item);
        txtEl.classList.remove("ss-ph");
    ...
    search.addEventListener("keydown", e => {
        ...
        if (e.key === "ArrowDown") {
            ...
            opts.forEach((o, i) => o.classList.toggle("ss-hi", i === hi));
    ...
    document.addEventListener("click", e => {
        const w = document.getElementById(cfg.wrapperId);
...
makeSearchableSelect({
    wrapperId: "clienteWrapper",
    mainId: "clienteMain",
    dropId: "clienteDropdown",
    searchId: "clienteSearch",
    optsId: "clienteOptions",
    txtId: "clienteText",
    items: clientesData,
        ...
        fetch("../presupuestos/get_contacto.php?client_id=" + cliente.id)
            ...
            .then(contactos => {
                if (contactos && contactos.length > 0) {
                    ...
                    if (fn && document.getElementById("clienteFirmante")) {
                        document.getElementById("clienteFirmante").value = fn;
                    }
                }
            })
            .catch(() => {});
...
if (selClienteId) {
    ...
    if (pre) {
        const t = document.getElementById("clienteText");
        t.textContent = pre.company_name || "";
        t.classList.remove("ss-ph");
    }
}

// ── EDITORES WYSIWYG POR DESCRIPCIÓN DE SERVICIO ───────────────
const itemDescQuills = {};

function initItemDescQuill(idx) {
    const editorEl = document.getElementById("itemDescEditor_" + idx);

    if (!editorEl || itemDescQuills[idx]) return;
    ...
    itemDescQuills[idx] = new Quill(editorEl, {
        theme: "snow",
        modules: {
            toolbar: [
    ...
    if (hidden && hidden.value && hidden.value.trim()) {
        itemDescQuills[idx].root.innerHTML = hidden.value;
    }

    itemDescQuills[idx].on("text-change", function () {
        if (!hidden) return;

        let html = itemDescQuills[idx].root.innerHTML;

        if (itemDescQuills[idx].getText().trim() === "") {
            html = "";
        }
...
function initArtSS(idx) {
    artSS[idx] = makeSearchableSelect({
        wrapperId: "artWrapper_" + idx,
        mainId: "artMain_" + idx,
        dropId: "artDropdown_" + idx,
        searchId: "artSearch_" + idx,
        optsId: "artOptions_" + idx,
        txtId: "artText_" + idx,
        items: articulosData,
        label: a => a.title || "",
        ...
        onSelect: art => {
            ...
            const descHtml = art.description || "";
            const hiddenDesc = row.querySelector(".item-descripcion-hidden");

            if (hiddenDesc) hiddenDesc.value = descHtml;

            if (itemDescQuills[idx]) {
                itemDescQuills[idx].root.innerHTML = descHtml;
            }
...
function addItem() {
    ...
    div.setAttribute("data-item-index", idx);
    ...
    div.innerHTML = `
        <div class="form-group">
            ...
            <div class="ss-wrapper" id="artWrapper_${idx}">
                <div class="ss-main" id="artMain_${idx}">
                    <span class="ss-selected ss-ph" id="artText_${idx}">-- Buscar en catálogo --</span>
                    <div class="ss-arrow"></div>
                </div>

                <div class="ss-dropdown" id="artDropdown_${idx}">
                    <div class="ss-search">
                        <input type="text" id="artSearch_${idx}" placeholder="Buscar artículo..." autocomplete="off">
                    </div>
                    <div class="ss-options" id="artOptions_${idx}"></div>
                </div>
            </div>
            ...
            <div class="wysiwyg-container item-desc-editor-wrap" style="margin-top:10px;">
                <div id="itemDescEditor_${idx}" class="item-desc-editor"></div>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="precio-original-wrap">
                <label style="font-size:11px;color:#888;font-weight:400;">Precio original sin descuento (€) — opcional</label>
                ...
                <div class="precio-original-preview" id="prevOriginal_${idx}" style="display:none;font-size:12px;color:#E91E8C;text-decoration:line-through;margin-top:4px;font-weight:600;"></div>
            </div>
...
function removeItem(btn) {
    if (document.querySelectorAll(".item-row").length > 1) {
        ...
        if (itemDescQuills[idx]) {
            delete itemDescQuills[idx];
        }

        calcTotals();
    } else {
        alert("Debe haber al menos un servicio en el contrato.");
    }
}
        ...
        po.addEventListener("input", function () {
            ...
            if (!isNaN(v) && v > 0) {
                if (prev) {
                    ...
                    prev.textContent = "Antes: " + v.toFixed(2).replace(".", ",") + " €";
                }
            } else {
...
document.getElementById("contratoForm").addEventListener("submit", function (e) {
    ...
    Object.keys(itemDescQuills).forEach(function (idx) {
        const row = document.querySelector(`[data-item-index="${idx}"]`);
        if (!row) return;
    ...
    var clausulasHtml = clausulasQuill.root.innerHTML;
    if (clausulasQuill.getText().trim() === "") clausulasHtml = "";
    document.getElementById("clausulasHidden").value = clausulasHtml;
    ...
    fetch("api.php", {
        ...
        .then(data => {
            if (data.success) {
                if (e.submitter.value === "guardar_pdf") {
