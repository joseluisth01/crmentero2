    ...
    if (file_exists($contratosFile)) {
        ...
        if ($contratoEliminar && !empty($contratoEliminar['crm_contract_id'])) {
            ...
            if ($mysqli) {
                ...
                guardarAuditoria('contrato_eliminado', 'exitoso',
        ...
        foreach ($blocks as $block) {
            ...
            if (empty($boldMatches[0])) {
                if ($isHeading || $startsStrong) {
                    $pdf->SetFont('Helvetica', 'B', 9.5); $pdf->SetTextColor(33, 33, 33);
                    $pdf->SetX(20); $pdf->MultiCell($cW, 5, $rawText, 0, 'L'); $pdf->Ln(1);
                } elseif ($isListItem) {
                    $pdf->SetFont('Helvetica', '', 8.5); $pdf->SetTextColor(51, 51, 51);
                    $pdf->SetX(24); $pdf->MultiCell($cW - 4, 4.5, chr(149) . ' ' . $rawText, 0, 'J');
                } else {
                    $pdf->SetFont('Helvetica', '', 8.5); $pdf->SetTextColor(51, 51, 51);
                    $pdf->SetX(20); $pdf->MultiCell($cW, 4.5, $rawText, 0, 'J');
                }
                $pdf->Ln(2);
            } else {
                $segs = []; $pos = 0; $fullText = $remaining;
                ...
                if ($allBold) {
                    $pdf->SetFont('Helvetica', 'B', 9.5); $pdf->SetTextColor(33, 33, 33);
                    $pdf->SetX(20); $pdf->MultiCell($cW, 5, $segs[0]['text'], 0, 'L'); $pdf->Ln(1);
                } else {
                    $pdf->SetTextColor(51, 51, 51);
                    foreach ($segs as $seg) { $pdf->SetFont('Helvetica', $seg['bold'] ? 'B' : '', 8.5); $pdf->Write(4.5, $seg['text']); }
                    $pdf->Ln(0); $pdf->MultiCell($cW, 4.5, '', 0, 'L');
                }
                $pdf->Ln(2);
            }
        }
